...
const TABLE_NAME = 'india_should_i_build_pages'
...
export { indiaShouldIBuildHubPath, indiaShouldIBuildPath } from '@/lib/pseoPaths'

/**
 * Fetch a single published india_should_i_build_pages row by slug.
 */
export async function getIndiaShouldIBuildBySlug(
  slug: string
  ...
  if (error) {
    console.error(
      '[india_should_i_build_pages] Fetch failed for slug "%s": %s',
      cleanSlug,
      error.message
    )
    return null
...
export async function getAllIndiaShouldIBuildSlugs(): Promise<string[]> {
  ...
  if (error) {
    console.error('[india_should_i_build_pages] Build slug fetch failed: %s', error.message)
    return []
  }
