...
const TABLE_NAME = 'india_market_saturation_pages'
...
export { indiaMarketSaturationHubPath, indiaMarketSaturationPath } from '@/lib/pseoPaths'

/**
 * Fetch a single published india_market_saturation_pages row by slug.
 */
export async function getIndiaMarketSaturationBySlug(
  slug: string
  ...
  if (error) {
    console.error(
      '[india_market_saturation_pages] Fetch failed for slug "%s": %s',
      cleanSlug,
      error.message
    )
    return null
...
export async function getAllIndiaMarketSaturationSlugs(): Promise<string[]> {
  const { data, error } = await supabaseAdmin
    .from('india_market_saturation_pages')
    .select('slug')
    .eq('is_published', true)

  if (error) {
    console.error('[india_market_saturation_pages] Build slug fetch failed: %s', error.message)
    return []
  }
