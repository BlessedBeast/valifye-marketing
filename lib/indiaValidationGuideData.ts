...
const TABLE_NAME = 'india_validation_guide_pages'
...
export { indiaValidationGuideHubPath, indiaValidationGuidePath } from '@/lib/pseoPaths'

/**
 * Fetch a single published india_validation_guide_pages row by slug.
 */
export async function getIndiaValidationGuideBySlug(
  slug: string
  ...
  if (error) {
    console.error(
      '[india_validation_guide_pages] Fetch failed for slug "%s": %s',
      cleanSlug,
      error.message
    )
    return null
...
export async function getAllIndiaValidationGuideSlugs(): Promise<string[]> {
  const { data, error } = await supabaseAdmin
    .from('india_validation_guide_pages')
    .select('slug')
    .eq('is_published', true)

  if (error) {
    console.error('[india_validation_guide_pages] Build slug fetch failed: %s', error.message)
    return []
  }
