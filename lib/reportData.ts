...
export interface ExperimentData {
  ...
  // Thick-data extensions are flexible; allow arbitrary keys without breaking existing callers
  [key: string]: unknown
}
...
export async function getReportBySlug(slug: string): Promise<ValidationReport | null> {
  ...
  if (error) {
    console.error('Supabase Fetch Error (report by slug):', error)
    return null
  }

  if (!data) return null

  const experiment_data = safeParseJSON<ExperimentData>(data.experiment_data)

  return {
    id: data.id,
    slug: data.slug,
    idea_title: data.idea_title ?? '',
    final_verdict: (data.final_verdict as VerdictType) ?? 'PIVOT',
...
export async function getReportsList(limit = 50): Promise<ValidationReport[]> {
  const { data, error } = await supabase
    .from('verdict_reports')
    .select('slug, idea_title, final_verdict, overall_integrity_score, created_at')
    .order('created_at', { ascending: false })
    .limit(limit)

  if (error) {
    console.error('Supabase Fetch Error (reports list):', error)
    return []
  }
  ...
  return data.map((row) => ({
    ...
    final_verdict: (row.final_verdict as VerdictType) ?? 'PIVOT',
    overall_integrity_score: Number(row.overall_integrity_score) ?? 0,
    forensic_narrative: null,
