...
const TABLE_NAME = 'india_local_opportunity_pages'
...
export { indiaLocalOpportunityHubPath, indiaLocalOpportunityPath } from '@/lib/pseoPaths'

/**
 * Fetch a single published india_local_opportunity_pages row by slug.
 */
export async function getIndiaLocalOpportunityBySlug(
  slug: string
  ...
  if (error) {
    console.error(
      '[india_local_opportunity_pages] Fetch failed for slug "%s": %s',
      cleanSlug,
      error.message
    )
    return null
...
export async function getAllIndiaLocalOpportunitySlugs(): Promise<string[]> {
  const { data, error } = await supabaseAdmin
    .from('india_local_opportunity_pages')
    .select('slug')
    .eq('is_published', true)

  if (error) {
    console.error('[india_local_opportunity_pages] Build slug fetch failed: %s', error.message)
    return []
  }
