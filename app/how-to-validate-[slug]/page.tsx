...
import {
  getValidationGuideBySlug,
  validationGuidePath,
  type MentionedTool,
  type ValidationGuidePage,
  type ValidationGuideStep
} from '@/lib/validationGuideData'
import {
  formatIsoDuration,
  generateHowToSchema
...
export async function generateStaticParams() {
  ...
  if (error) {
    console.error('Build fetch failed for validation_guide_pages:', error)
    return []
  }
...
function pageUrl(slug: string): string {
  return `${SITE_URL}${validationGuidePath(slug)}`
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const row = await getValidationGuideBySlug(slug)
...
export default async function ValidationGuidePage({ params }: Props) {
  ...
  console.log(`[PSEO DEBUG] table: validation_guide_pages | raw params:`, resolvedParams)
