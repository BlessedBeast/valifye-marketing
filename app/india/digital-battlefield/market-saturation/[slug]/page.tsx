...
import {
  getAllIndiaMarketSaturationSlugs,
  getIndiaMarketSaturationBySlug,
  indiaMarketSaturationPath,
  type MarketSaturationPage,
  type MarketSaturationPlayer
} from '@/lib/indiaMarketSaturationData'
...
export async function generateStaticParams() {
  const slugs = await getAllIndiaMarketSaturationSlugs()
...
function pageUrl(slug: string): string {
  return `${SITE_URL}${indiaMarketSaturationPath(slug)}`
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const row = await getIndiaMarketSaturationBySlug(slug)
...
export default async function MarketSaturationPage({ params }: Props) {
  ...
  console.log(`[PSEO DEBUG] table: india_market_saturation_pages | raw params:`, resolvedParams)
