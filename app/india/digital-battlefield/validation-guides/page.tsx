...
import { indiaValidationGuidePath } from '@/lib/indiaValidationGuideData'
...
export const metadata: Metadata = {
  ...
  alternates: { canonical: `${SITE_URL}/india/digital-battlefield/validation-guides` },
}
...
export default async function ValidationGuidesHubPage() {
  ...
  if (error) {
    console.error('[india/digital-battlefield/validation-guides hub] Fetch failed:', error.message)
  }
  ...
  return (
    <MarketingShell className="max-w-6xl gap-10">
      ...
      <PseoHubGrid
        items={rows.map((row) => ({
          href: indiaValidationGuidePath(row.slug),
