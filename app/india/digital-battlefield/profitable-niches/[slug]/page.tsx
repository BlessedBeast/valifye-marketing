...
import {
  getAllIndiaProfitableNicheSlugs,
  getIndiaProfitableNicheBySlug,
...
export async function generateStaticParams() {
  const slugs = await getAllIndiaProfitableNicheSlugs()
...
function pageUrl(slug: string): string {
  return `${SITE_URL}${indiaProfitableNichePath(slug)}`
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const row = await getIndiaProfitableNicheBySlug(slug)
...
export default async function ProfitableNichePage({ params }: Props) {
  ...
  console.log(`[PSEO DEBUG] table: india_profitable_niche_pages | raw params:`, resolvedParams)
