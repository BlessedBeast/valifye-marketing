...
import { indiaLocalOpportunityPath } from '@/lib/indiaLocalOpportunityData'
...
export const metadata: Metadata = {
  ...
  alternates: { canonical: `${SITE_URL}/india/digital-battlefield/local-opportunities` },
}
...
export default async function LocalOpportunitiesHubPage() {
  ...
  if (error) {
    console.error('[india/digital-battlefield/local-opportunities hub] Fetch failed:', error.message)
  }
  ...
  return (
    <MarketingShell className="max-w-6xl gap-10">
      ...
      <PseoHubGrid
        items={rows.map((row) => ({
          href: indiaLocalOpportunityPath(row.slug),
