...
const solutionLinks = [
  {
    ...
    description: 'Forensic diligence before you lock capital in brick & mortar.',
    Icon: FileSignature,
    ...
    <header className="relative z-40 flex h-16 items-center justify-between border-b border-border bg-background px-6 font-mono">
      ...
      <nav className="hidden md:flex items-center gap-6">
        ...
        <div className="group relative">
          <button
            type="button"
            className="inline-flex items-center gap-2 border border-border bg-card px-3 py-1 text-xs font-bold uppercase tracking-widest text-muted-foreground transition-colors hover:border-amber-500/50 hover:text-amber-200 group-focus-within:border-amber-500/50 group-focus-within:text-amber-200"
            aria-haspopup="true"
          >
          ...
          <div className="invisible absolute right-0 top-full z-50 mt-3 w-[min(100vw-2rem,380px)] translate-y-2 opacity-0 transition-all duration-150 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100">
            <div className="overflow-hidden rounded-lg border border-zinc-800/90 bg-slate-950/90 p-2 shadow-[0_20px_80px_-24px_rgba(0,0,0,0.9),0_0_40px_-18px_rgba(245,158,11,0.35)] backdrop-blur-xl">
              ...
              <div className="py-2">
                {solutionLinks.map(({ href, label, description, Icon, accent }) => (
                  <Link
                    key={href}
                    href={href}
